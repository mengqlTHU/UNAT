...
//	addSingleArray(refFrontEdgeData, 1, NONZERONUM, COPYIN, lower_DSI);
    constructSingleArray(refSelfConnData, dims, vertexNum, COPYIN,
				diag_DSI);
...
//	constructEmptyArray(refVertexData);
    ...
    addSingleArray(refVertexData, dims, vertexNum, COPYIN, x_DSI);
...
//    constructSingleArray(vertexData, dims, vertexNum, COPYIN, x);
    addSingleArray(vertexData, dims, vertexNum, COPYIN, x);
//    constructSingleArray(refVertexData, dims, vertexNum, COPYOUT, b);
//    addSingleArray(refVertexData, dims, vertexNum, COPYOUT, b);


	MultiLevelBlockIterator mlbIter(*topo,
				&cellWeights[0], &edgeWeights[0]);
//	mlbIter.reformInnerTopology();
	mlbIter.reorderEdgeData(&backEdgeData, &frontEdgeData);
//	mlbIter.reorderEdgeData(&refBackEdgeData, &refFrontEdgeData);
	mlbIter.reorderVertexData(&vertexData);
//	mlbIter.reorderVertexData(&refVertexData);
	mlbIter.reorderVertexData(&selfConnData);
//	mlbIter.reorderVertexData(&refSelfConnData);
...
//	// Timer
	...
	spMV(&refBackEdgeData,&refFrontEdgeData,&refSelfConnData,
...
////	}
	...
	CG_init();
//	mlbIter.edge2VertexIteration(&backEdgeData, &frontEdgeData,
//				&selfConnData, &vertexData,
