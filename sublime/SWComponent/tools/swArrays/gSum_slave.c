...
void gSum_slave(MVM_ParametersPack* MVM_ParametersPack_hostPtr)
{
    ...
    LABEL i, j, remaining, offset_vector = sPosition, nReturns;
    sizePerCycle = (lenLocal < sizePerCycle)? lenLocal : sizePerCycle;

    SCALAR sum1_slave = 0.0, sum2_slave = 0.0;
    ...
    if(MVM_ParametersPack_slave.k1Ptr && MVM_ParametersPack_slave.k2Ptr)
    {
        nReturns = 2;
    }
    else
    ...
    MVM_Arrays_slave.k1Ptr = &sum1_slave;
    MVM_Arrays_slave.k2Ptr = &sum2_slave;
    ...
    sendinfo[0] = sum1_slave;
    recvinfo[0] = sum1_slave;

    sendinfo[1] = sum2_slave;
    recvinfo[1] = sum2_slave;
