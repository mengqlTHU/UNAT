...
void userFunc_aEaMib(MVM_Arrays *MVM_ArraysPtr)
{
	// a -= b
	SCALAR *A1Ptr;
	SCALAR *A2Ptr;
...
void userFunc_aEaDb(MVM_Arrays *MVM_ArraysPtr)
{
	// a -= b
	SCALAR *A1Ptr;
	SCALAR *A2Ptr;
...
void userFunc_residualSumKSqr(MVM_Arrays *MVM_ArraysPtr)
{
	// a = b - k1 * c
	// k = Sum(fabs(a))
	SCALAR *A1Ptr;
	...
	{
		A1Ptr[i]  = A2Ptr[i] - k1 * A3Ptr[i];
		k1Ptr[0] += (A2Ptr[i] - k1 * A3Ptr[i]) * (A2Ptr[i] - k1 * A3Ptr[i]);
	}
}
...
void userFunc_sumSqrDot(MVM_Arrays *MVM_ArraysPtr)
{
	// k1 = Sum(b * b)
	// k2 = Sum(b * c)
	SCALAR *A2Ptr;
	...
	A3Ptr = MVM_ArraysPtr->A3Ptr;
	SCALAR *k1Ptr = MVM_ArraysPtr->k1Ptr;
	SCALAR *k2Ptr = MVM_ArraysPtr->k2Ptr;
	LABEL i, n = MVM_ArraysPtr->nCells;

	for(i=0; i<n; i++)
	{
		k1Ptr[0] += A2Ptr[i] * A2Ptr[i];
		k2Ptr[0] += A2Ptr[i] * A3Ptr[i];
	}
}
